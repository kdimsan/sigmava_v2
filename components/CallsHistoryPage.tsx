...
export default function CallsHistoryPage({ appointments }: Appointments) {
  ...
  const appointmentsFilter = appointments.filter((ap => ap.status === "cancelled" || ap.status === "completed"))

  return (
    <div className="max-w-4xl mx-auto space-y-4">
      {appointmentsFilter.map((appointment)  => (
        <AppointmentCard
          key={appointment.id}
